...
def create_data_fram(data, crystal , ID, date):
    ...
    col = len(columns)-2
    click = 1
    n = int(len(data)/(11*col))
    data = np.array(data).reshape(col*n,11)
    data_neat = []
    for i in range(col):
        temp = data[i::col,:]
        data_neat = np.append(data_neat,temp)
    data_neat = data_neat.reshape(col,11*n).T
    ...
    for f, d in zip(sorted(file_list, key = first_3chars)  ,data_neat) :
        ...
        M = np.hstack((M,d.T))
    print(n)
    Result = pd.DataFrame(M.reshape(1600*11*n,col+2), columns=["# mesurment","ex_wl","spectro_center_wl",'grating','spf','lpf','lpf2',"av_power",
    ...
    for c in crystal:
        ...
        for wl in wlrange[::-1]:
            ...
            for state in set_up_list:
                ...
                data.extend([spectro_grating for number in range(11)])
                data.extend([spf for number in range(11)])
                data.extend([lpf for number in range(11)])
                data.extend([lpf2 for number in range(11)])
